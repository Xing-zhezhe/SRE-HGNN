...
class JSONFileDataLoader(FileDataLoader):
    ...
    def sen_wor_to_graph(self, B, N, K, Q, support_word, support_head, support_tail, query_word, query_head, query_tail):
        ...
        adj = np.zeros((class_num, 3 * (1 + K * N), 3 * (1 + K * N)), dtype=np.int)#1 query + K * N support
        ...
        for i in range (class_num):
            ...
            for j in range(1 + K * N):
                ...
                adj[i][j][(1 + K * N) * 2 + j] = 1
                adj[i][(1 + K * N) * 2 + j][j] = 1
                adj[i][1 + K * N + j][(1 + K * N) * 2 + j] = 1
                adj[i][(1 + K * N) * 2 + j][1 + K * N + j] = 1
            for l in range(1, 6):
    ...
    def next_batch(self, B, N, K, Q, noise_rate=0):
        ...
        # print("9999999999999999999999000000000000000000000")
        # input(x)
        return adj, sen_adj, entity_adj, total_graph, sen_graph, entity_graph, support, query, label
