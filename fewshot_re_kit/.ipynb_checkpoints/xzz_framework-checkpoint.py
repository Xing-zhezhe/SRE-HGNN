...
class FewShotREFramework:
    ...
    def train(self,
        ...
        for it in range(start_iter, start_iter + train_iter):
            scheduler.step()
            adj, sen_adj, entity_adj, total_graph, sen_graph, entity_graph, support, query, label = self.train_data_loader.next_batch(B, N_for_train, K, Q, noise_rate=noise_rate)
            logits, pred = model(adj, sen_adj, entity_adj, total_graph, sen_graph, entity_graph, support, query, B, N_for_train, K, Q ,label, model, is_training=True)
            # print(logits)
            # print(pred)
            # print(label)
            # input(ooo)
    ...
    def eval(self,
        ...
        for it in range(eval_iter):
            adj, sen_adj, entity_adj, total_graph, sen_graph, entity_graph, support, query, label = eval_dataset.next_batch(B, N, K, Q, noise_rate=noise_rate)
            logits, pred = model(adj, sen_adj, entity_adj, total_graph, sen_graph, entity_graph, support, query, B, N, K, Q ,label, model, is_training=False )
